...
The total time spent are around 5 hours.
...
- **Input validation on `Execute`** — the current implementation doesn't guard against null orders in the list or non-positive volume/notional values. In a production trading context these are exactly the kind of malformed inputs that can slip through from upstream systems.
...
4. **Fix and verify against the original signal** — after patching, I confirm against the same metric that originally flagged the issue, not just a subjective "seems faster now."
